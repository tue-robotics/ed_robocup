...
void Segmenter::segment(const ed::WorldModel& world, const rgbd::Image& image, const geo::Pose3D& sensor_pose, std::vector<Segment>& segments)
{
    // - - - - - - - - - - - - - - - - - -
...
//                    associates = associates || pointAssociates(p, *pc_model, x2, y2, min_dist_sq);
    ...
    for(unsigned int i = 0; i < non_assoc_mask.size(); ++i)
    {
        ...
        if (pixels.size() < 100) // TODO: magic number
        {
            segments.pop_back();
            continue;
        }
        ...
        std::vector<geo::Vec2f> points_2d(pixels.size());
        for(unsigned int j = 0; j < pixels.size(); ++j)
        {
            const pcl::PointNormal& p = pc->points[pixels[j]];
